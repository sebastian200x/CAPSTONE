...
# "D:/Programming/faceRecognition_files/dataset/haarcascade_frontalface_default"
...
def train_classifier(nbr):
    dataset_dir = 'resources/haarcascade_frontalface_default.xml'
...
def face_recognition():
    def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
        ...
        for (x, y, w, h) in features:
            ...
            select.execute("SELECT b.prs_name FROM img_dataset a LEFT JOIN prs_mstr b ON a.img_person = b.prs_nbr WHERE img_id = %s", (id,))
            s = select.fetchone()
            s = '' + ''.join(s)
            if confidence > 70:
...
def home():
    select = mysql.connection.cursor()
    select.execute("select prs_nbr, prs_name, prs_skill, prs_active, prs_added from prs_mstr")
